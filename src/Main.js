    ...
    render() {
        const date = () => {
            if (this.state.date.d !== undefined) {
                ...
                return (<p>Time together: <span className="date-text">{d}</span> hour <span className="date-text">{hour}</span> minute <span className="date-text">{minute}</span> second <span className="date-text">{second}</span> 秒 </p>
                )
            }
        }
        return (
